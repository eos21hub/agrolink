...
export function WeatherWidget({ location = 'Accra,GH', compact = false }: WeatherWidgetProps) {
  ...
  if (error || !weather) {
    return (
      <div className="card text-center py-4 text-blue-400 text-sm">
    ...
    return (
      <div className="card">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-display font-bold text-2xl text-blue-900">{weather.temperature}°C</div>
            <div className="text-xs text-blue-600 capitalize">{weather.description}</div>
            <div className="text-xs text-blue-400">{weather.location}</div>
          </div>
  ...
  return (
    <div className="card">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="font-display font-bold text-blue-400 text-xs uppercase tracking-widest">Weather</h3>
          <p className="text-xs text-blue-400 mt-0.5">{weather.location}</p>
        </div>
        <img src={iconUrl} alt={weather.description} className="w-14 h-14" />
      </div>

      <div className="flex items-end gap-2 mb-4">
        <span className="font-display font-bold text-5xl text-blue-900">{weather.temperature}°</span>
        <div className="mb-1.5">
          <div className="text-sm text-blue-700 capitalize font-semibold">{weather.description}</div>
          <div className="text-xs text-blue-400">Feels like {weather.feels_like}°C</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ...
          <div key={label} className="bg-blue-50 rounded-xl p-2.5 border border-blue-100 text-center">
            ...
            <div className="font-mono font-bold text-sm text-blue-800">{value}</div>

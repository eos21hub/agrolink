    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {!loading && prices.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Total Entries',    value: prices.length,                              color: 'text-indigo-600' },
          ...
          <table className="w-full text-sm">
              ...
              <tr className="border-b border-blue-100 bg-blue-50">
                {([
                  ...
                  <th
                    ...
                    className="px-5 py-3.5 text-left text-xs font-display font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-indigo-700 transition-colors"
                  >
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="px-5 py-12 text-center text-slate-300 text-sm">{query ? 'No results found' : 'No market price data available'}</td></tr>
              ) : sorted.map((price, i) => (
                <tr key={price.id} className={`${i !== sorted.length - 1 ? 'border-b border-blue-50' : ''} hover:bg-blue-50/70 transition-colors`}>
                  <td className="px-5 py-3.5 font-semibold text-indigo-950 capitalize">{price.crop}</td>
                  <td className="px-5 py-3.5 text-slate-500">{price.market}</td>
                  <td className="px-5 py-3.5">
                    <span className="font-mono font-bold text-indigo-700">GHS {price.price_per_kg}</span>
                  </td>
